...
class Woo_Facturante {
	...
	public function handle_callback(){
		...
		else	
		{
				...
				//Obtengo el xml ya sea desde post o directamente desde xml
				
				( isset( $_POST['DetalleComprobante']) )? $xmlaux = $_POST['DetalleComprobante'] : $xmlaux = file_get_contents('php://input');
				
				$xml = simplexml_load_string(preg_replace('/(<\?xml[^?]+?)utf-16/i', '$1utf-8',stripslashes($xmlaux)));
				...
				switch($xml->EstadoComprobante){
					...
					default:
							...
							echo 'Estado no contemplado o xml inválido '.var_dump($xml);
					break; 
