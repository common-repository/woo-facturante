    ...
    public static function get_settings() {
        ...
        $settings = array(
            ...
            'user' => array(
                ...
                'type' => 'text',
                'desc' => 'Usuario provisto por <a href="https://facturante.com" target="_blank">facturante.com</a>',
            ...
            'hash' => array(
                ...
                'type' => 'password',
                'desc' => 'Clave provista por <a href="https://facturante.com" target="_blank">facturante.com</a>',
			...
			'organization_ID' => array(
                ...
                'type' => 'text',
                'desc' => 'Id de empresa provisto por <a href="https://facturante.com" target="_blank">facturante.com</a>',
                'id'   => 'wc_settings_tab_woo_facturante_organization_ID',
				'custom_attributes' =>array("required"=>"required","oninvalid"=>"this.setCustomValidity('Debe solicitar este dato a Facturante')","oninput"=>"setCustomValidity('')"),
				/*'tooltip_html'=>'hola!',*/
				'desc_tip'=>false,
				'suffix'=>'',
	...
	public function woo_facturante_order_actions($actions,$the_order){
		...
		if ( !$the_order->has_status( array( 'cancelled' ) ) && ( $the_order->has_status( array( 'processing' ) ) || $the_order->has_status( array( 'completed' ) ) ) ) { 
			...
			if($invoice_id==''){
				...
				if( !is_plugin_active('woo-facturante-exportacion/woo-facturante-exportacion.php') ):
	...
	**/
	...
	public function woo_facturante_invoice(){
		...
		*/
		...
		$auth = apply_filters('auth_data',$auth);
		...
		$cliente = apply_filters('cliente_data',$cliente);
		...
		$encabezado = array(
						...
						"Moneda" => 2, // PESOS ARGENTINOS (2 ARS, 3 DOLAR USA) // TODO
							
						"Prefijo" => get_option('wc_settings_tab_woo_facturante_prefix'),
					
						"TipoComprobante" => 'F', //TODO
		...
		$encabezado = apply_filters('encabezado_data',$encabezado);
		...
		try {
			  ...
			  $request_function = 'CrearComprobanteSinImpuestos';
			  $response_function = 'CrearComprobanteSinImpuestosResult';
			  
			  $request_function = apply_filters('change_request_func',$request_function);
			  $response_function = apply_filters('change_response_func',$response_function);
			
			  $response = $client->$request_function($request);
			    
			  if ( isset( $response->$response_function->IdComprobante ) ) {
				  
				update_post_meta( $order_id, '_invoice_id', $response->$response_function->IdComprobante );
